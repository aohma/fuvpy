...
ds = xr.load_dataset(path + 'sp_mlatmlt_order0.nc')
ds2 = xr.load_dataset(path + 'sp_binnumber_order0.nc')
...
for t in range(len(ds2.rind)):
    fig,ax = plt.subplots()
    pax = pp(ax)
    fc = pax.filled_cells(grid[0,:],grid[1,:],2,mltres,ds2.isel(rind=t)['mean'].values,crange=(0,400))
    plt.colorbar(fc)
    ...
    ds.isel(rind=t)['mean'].plot(vmin=0,vmax=600)
...
fig,ax = plt.subplots()
ds.sel(rlat=slice(-10,10))['mean'].mean(dim='rlat').plot(x='rlt',y='rind',vmin=0,vmax=400)
